locals {
  service_bus = var.service_bus_staging.use_sb_test ? {
    id   = data.azurerm_servicebus_namespace.test_sb.id
    name = data.azurerm_servicebus_namespace.test_sb.name
    } : {
...
data "azurerm_servicebus_namespace" "test_sb" {
  name                = var.service_bus_staging.name
...
resource "azurerm_servicebus_namespace" "main" {
  ...
  count = var.service_bus_staging.use_sb_test ? 0 : 1
...
resource "azurerm_private_endpoint" "sb_main" {
  count = var.service_bus_config.sku == "Premium" && var.service_bus_staging == false ? 1 : 0
