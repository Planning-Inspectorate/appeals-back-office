# second web app for the test environment while deployment issues are investigated
module "app_web_2" {
  ...
  app_settings = {
    ...
    APPEALS_READ_ONLY_GROUP_ID    = var.apps_config.auth.group_ids.read_only
...
## RBAC for secrets
resource "azurerm_role_assignment" "app_web_2_secrets_user" {
  count = var.environment == "test" ? 1 : 0

  scope                = azurerm_key_vault.main.id
  role_definition_name = "Key Vault Secrets User"
  principal_id         = module.app_web_2[0].principal_id
}
