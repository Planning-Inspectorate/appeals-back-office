...
- do we even need a config folder, policies can be held elsewhere to keep repo smaller and ensure policies are same through all services.

## Extensions

- tflint
- took off -no-color in `terraform validate -no-color` to experiment if any issues
...
The state files will be hosted in the tooling sub.
The repo is infra-tooling. We won't need to use it again now the storage is there.

The SP for each environment will have storage blob contributor on its relevant state file container in the tooling-sub. That SP will also have contributor rights on the env where all the TF will live in the ODT sub.

- Changed the checkov and tflint file names so each are located next to their plugin
...
- OR LINK TO CONFLUENCE PAGE INSTALLING ALL STEPS TO SAVE SPACE HERE

tfenv(non-Windows) is an optional tool enabling the installation of multiple versions of Terraform. It is useful in case the developer is working on seveal Terraform projects pinned to specific Terraform versions:

```bash
brew install tfenv
tfenv install 1.2.6 # check for version in use
```

#### Install Pre-Commit Hooks

This project supports the use of Pre-Commit(not all the tools can be used with Windows). Pre-Commit is a tool that can run checks against your code upon committing changes to a branch. Checks need to pass, otherwise the changes will not be committed. Some checks will automatically fix files (e.g. trailing whitespace). In particular, this tool supports Terraform-specific checks to ensure Terraform code is formatted correctly and executable:
...
// needs to be changed as the way the state is called is now different
