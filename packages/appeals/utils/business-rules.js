...
export function getNextStateOnStatementsComplete(appealType, procedureType, isHearingSetUp){
	...
	const nextState = targetStateOnStatementsComplete(isLdcOrDiscontinuanceOrEnforcementAppealType(appealType), normalisedProcedureType);
	...
	const eventualState = (isHearingSetUp && nextState === APPEAL_CASE_STATUS.EVENT) ? APPEAL_CASE_STATUS.AWAITING_EVENT : nextState;
...
// Note that this is used in the state machine and so this may not be the final state it ends up in, but the next state after the event is cancelled.
