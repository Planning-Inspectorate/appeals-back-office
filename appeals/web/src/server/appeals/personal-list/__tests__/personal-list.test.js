...
describe('personal-list', () => {
	...
	describe('mapActionLinksForAppeal', () => {
		...
		describe.each([
			...
			])('should return progressFromStatements link with correct copy when procedure type is %s and hearing set up is %s and there are no statements or comments', (procedureType, hearingSetUp, expectedText) => {
				const appealData =
					appealDataToGetRequiredActions['progressFromStatements'];

				const expectedHtml =  {
					caseOfficer: `<a class="govuk-link" href="/appeals-service/appeal-details/${appealData.appealId}/share?backUrl=%2Fappeals-service%2Fpersonal-list">Progress to ` + expectedText + `<span class="govuk-visually-hidden"> for appeal ${appealData.appealId}</span></a>`
				}
