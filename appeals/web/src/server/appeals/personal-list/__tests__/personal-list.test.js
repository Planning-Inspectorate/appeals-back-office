	...
	describe('mapActionLinksForAppeal', () => {
		...
		const testCases = [
			...
			},
			{
				...
				expectedHtml: {
					caseOfficer: `<a class="govuk-link" href="/appeals-service/appeal-details/${appealId}/share">Progress case</a>`
				}
			},
			{
				...
				expectedHtml: {
					caseOfficer: `<a class="govuk-link" href="/appeals-service/appeal-details/${appealId}/share">Progress to final comments<span class="govuk-visually-hidden"> for appeal ${appealId}</span></a>`
				}
			},
			{
			...
			},
			{
				...
				expectedHtml: {
					caseOfficer: `<a class="govuk-link" href="/appeals-service/appeal-details/${appealId}/interested-party-comments">Review IP comments<span class="govuk-visually-hidden"> for appeal ${appealId}</span></a>`
				}
			},
			{
			...
			},
			{
				...
				expectedHtml: {
					caseOfficer: `<a class="govuk-link" href="/appeals-service/appeal-details/${appealId}/lpa-statement">Review LPA statement<span class="govuk-visually-hidden"> for appeal ${appealId}</span></a>`
				}
			},
			{
				...
				expectedHtml: {
					caseOfficer: `<a class="govuk-link" href="/appeals-service/appeal-details/${appealId}/share">Share final comments</a>`
				}
			},
			{
				...
				expectedHtml: {
					caseOfficer: `<a class="govuk-link" href="/appeals-service/appeal-details/${appealId}/share">Share IP comments and LPA statement<span class="govuk-visually-hidden"> for appeal ${appealId}</span></a>`
				}
			},
			{
				name: 'Start case',
		...
		for (const testCase of testCases) {
			it(`should return the expected "${testCase.name}" action link HTML if getRequiredActionsForAppeal returns "${testCase.requiredAction}" and "isCaseOfficer" is true`, async () => {
				jest.unstable_mockModule('#lib/mappers/utils/required-actions.js', () => ({
					getRequiredActionsForAppeal: jest.fn().mockReturnValue([testCase.requiredAction])
				}));
				const { mapActionLinksForAppeal } = await import(
					'#appeals/personal-list/personal-list.mapper.js'
				);

				const result = mapActionLinksForAppeal(mockAppealData, true);

				expect(result).toBe(testCase.expectedHtml.caseOfficer);
			});

			if ('nonCaseOfficer' in testCase.expectedHtml) {
				it(`should return the expected "${testCase.name}" action link HTML if getRequiredActionsForAppeal returns "${testCase.requiredAction}" and "isCaseOfficer" is false`, async () => {
					jest.unstable_mockModule('#lib/mappers/utils/required-actions.js', () => ({
						getRequiredActionsForAppeal: jest.fn().mockReturnValue([testCase.requiredAction])
					}));
					const { mapActionLinksForAppeal } = await import(
					...
					const result = mapActionLinksForAppeal(mockAppealData, false);
