 ...
 * @returns {PageContent}
 */
export function cancelInquiryPage(appealData) {
	const shortAppealReference = appealShortReference(appealData.appealReference);

	const paragraphComponent = simpleHtmlComponent(
		'p',
		{ class: 'govuk-body' },
		`We’ll send an email to the appellant and LPA to tell them the inquiry has been cancelled.`
	);
	...
	/** @type {PageContent} */
	const pageContent = {
		title: `Confirm that you want to cancel the inquiry - ${shortAppealReference}`,
		backLinkUrl: `/appeals-service/appeal-details/${appealData.appealId}`,
		preHeading: `Appeal ${shortAppealReference}`,
		heading: 'Confirm that you want to cancel the inquiry',
		pageComponents: [paragraphComponent],
		postPageComponents: [keepHearingLink],
		submitButtonText: 'Cancel inquiry',
