...
export const postEditTimetable = async (request, response) => {
	...
	timeTableTypes.forEach((timetableType) => {
		...
		session.appealTimetables[timetableType] = dayMonthYearHourMinuteToISOString({
...
export const renderCheckYourAnswers = (request, response) => {
	...
	if (!objectContainsAllKeys(session, 'appealTimetables')) {
		return response.status(500).render('app/500.njk');
	}
	const appealTimetables = session.appealTimetables;
	const currentDueDateIso = appealTimetables && appealTimetables['lpaQuestionnaireDueDate'];
