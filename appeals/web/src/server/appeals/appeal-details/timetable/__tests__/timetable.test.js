...
describe('Timetable', () => {
	...
	describe('GET /edit', () => {
		...
		describe.each([APPEAL_TYPE.S78, APPEAL_TYPE.PLANNED_LISTED_BUILDING])(
			'S78 and S20',
			(appealType) => {
				it(`should render correct "Timetable due dates" page for ${appealType} type and status lpa_questionnaire`, async () => {
					const appealData = {
						...baseAppealData,
						appealTimetable: {
							appealTimetableId: 1
				...
				it(`should render correct "Timetable due dates" page for ${appealType} type and status lpa_questionnaire and lpaq received`, async () => {
					const appealData = {
						...baseAppealData,
						appealTimetable: {
							appealTimetableId: 1
				...
				it(`should render correct "Timetable due dates" page for ${appealType} type and status statements`, async () => {
					const appealData = {
						...baseAppealData,
						appealTimetable: {
							appealTimetableId: 1
				...
				it(`should render correct "Timetable due dates" page for ${appealType} type and status final_comments`, async () => {
					const appealData = {
						...baseAppealData,
						appealTimetable: {
							appealTimetableId: 1
				...
				it(`should render correct "Timetable due dates" page for ${appealType} type hearing appeal without planning obligation`, async () => {
					const appealData = {
						...baseAppealData,
				...
				it(`should render correct "Timetable due dates" page for ${appealType} type hearing appeal with planning obligation`, async () => {
					const appealData = {
						...baseAppealData,
						appealStatus: 'lpa_questionnaire',
						appealTimetable: {
				...
				it(`should render correct "Timetable due dates" page for ${appealType} type inquiry appeal without planning obligation`, async () => {
					const appealData = {
						...baseAppealData,
						appealStatus: 'lpa_questionnaire',
						appealTimetable: {
					...
					expect(element.innerHTML).toContain(
						'name="proof-of-evidence-and-witnesses-due-date-day"'
					);
					expect(element.innerHTML).toContain(
						'name="proof-of-evidence-and-witnesses-due-date-month"'
					);
					expect(element.innerHTML).toContain(
				...
				it(`should render correct "Timetable due dates" page for ${appealType} type inquiry appeal with planning obligation`, async () => {
					const appealData = {
						...baseAppealData,
						appealStatus: 'lpa_questionnaire',
						appealTimetable: {
			...
			(appealType) => {
				...
				const getBaseValidS78Payload = () => ({
					...
					'final-comments-due-date-month':
						baseValidS78PayloadParts['final-comments-due-date'].month,
				...
				const testCases = [
					{
						...
						expectedError: (/** @type {string} */ label) =>
							`${label} due date must include a day</a>`
					},
					{
						name: 'missing month',
						payload: (/** @type {string} */ id) => ({
			...
			(appealType) => {
				...
				const getBaseValidHearingPayload = () => ({
					...
					'lpa-statement-due-date-year':
						baseValidHearingPayloadParts['lpa-statement-due-date'].year,
		...
		describe.each([APPEAL_TYPE.S78, APPEAL_TYPE.PLANNED_LISTED_BUILDING])(
			...
			(appealType) => {
				...
				const baseValidHearingPayloadParts = {
					...
					'statement-of-common-ground-due-date': { day: '04', month: '10', year: '2050' },
					'planning-obligation-due-date': { day: '05', month: '10', year: '2050' },
					'proof-of-evidence-and-witnesses-due-date': { day: '06', month: '10', year: '2050' }
				};
				 ...
				 * 'planning-obligation-due-date-month': string,
				 * 'planning-obligation-due-date-year': string,
				 * 'proof-of-evidence-and-witnesses-due-date-day': string,
				 * 'proof-of-evidence-and-witnesses-due-date-month': string,
				 * 'proof-of-evidence-and-witnesses-due-date-year': string
				 * }}
				 */
				const getBaseValidHearingPayload = () => ({
					'lpa-questionnaire-due-date-day':
					...
					'proof-of-evidence-and-witnesses-due-date-day':
						baseValidHearingPayloadParts['proof-of-evidence-and-witnesses-due-date'].day,
					'proof-of-evidence-and-witnesses-due-date-month':
						baseValidHearingPayloadParts['proof-of-evidence-and-witnesses-due-date'].month,
					'proof-of-evidence-and-witnesses-due-date-year':
						baseValidHearingPayloadParts['proof-of-evidence-and-witnesses-due-date'].year
				});
				...
				const testCases = [
					{
						...
						expectedError: (/** @type {string} */ label) => `${label} due date must include a day`
					},
					{
						name: 'missing month',
						payload: (/** @type {string} */ id) => ({
						...
						expectedError: (/** @type {string} */ label) => `${label} due date must include a month`
					},
					{
						name: 'missing year',
						payload: (/** @type {string} */ id) => ({
						...
						expectedError: (/** @type {string} */ label) => `${label} due date must include a year`
					},
					{
						name: 'not a real date',
						payload: (/** @type {string} */ id) => ({
						...
						expectedError: (/** @type {string} */ label) => `${label} due date must be a real date`
					},
					{
						name: 'must be in the future',
						payload: (/** @type {string} */ id) => ({
						...
						expectedError: (/** @type {string} */ label) =>
							`The ${label.toLowerCase()} due date must be in the future`
					}
				];
					...
					describe(`${label}`, () => {
						it.each(testCases)(
							...
							async (testCase) => {
								...
								expect(element.querySelector('.govuk-error-summary__list li a')?.innerHTML).toBe(
									testCase.expectedError(label)
								);
							}
						);
					});
	...
	describe('GET /edit/check', () => {
		...
		describe.each([APPEAL_TYPE.S78, APPEAL_TYPE.PLANNED_LISTED_BUILDING])(
			'S78 and S20',
			(appealType) => {
				it(`should render correct "Timetable CYA" page for ${appealType} type and status lpa_questionnaire and lpaq NOT received`, async () => {
					const appeal = {
						...baseAppealData,
						appealTimetable: {
							appealTimetableId: 1
				...
				it(`should render correct "Timetable CYA" page for ${appealType} type and status lpa_questionnaire and lpaq received`, async () => {
					const appeal = {
						...baseAppealData,
						appealTimetable: {
							appealTimetableId: 1
				...
				it(`should render correct "Timetable CYA" page for ${appealType} type and status statements`, async () => {
					const appeal = {
						...baseAppealData,
						appealTimetable: {
							appealTimetableId: 1
				...
				it(`should render correct "Timetable CYA" page for ${appealType} type and status final_comments`, async () => {
					const appeal = {
						...baseAppealData,
						appealTimetable: {
							appealTimetableId: 1
				...
				it(`should render correct "Timetable CYA" page for ${appealType} type hearing appeal without planning obligation`, async () => {
					const appeal = {
						...baseAppealData,
						appealStatus: 'lpa_questionnaire',
						appealTimetable: {
				...
				it(`should render correct "Timetable CYA" page for ${appealType} type hearing appeal with planning obligation`, async () => {
					const appeal = {
						...baseAppealData,
						appealStatus: 'lpa_questionnaire',
						appealTimetable: {
			...
			it('should update the timetable due dates via the API', async () => {
				const apiCall = nock('http://test/')
					.patch('/appeals/1/appeal-timetables/1', {
						...
						proofOfEvidenceAndWitnessesDueDate: '2030-10-18T23:00:00.000Z'
					})
					.reply(200, {})
					.persist();
				...
				expect(response.statusCode).toBe(302);
				expect(response.headers.location).toBe('/appeals-service/appeal-details/1');
