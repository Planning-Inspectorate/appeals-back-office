 ...
 * @param {SessionWithAuth & { acceptAppellantStatement?: { [key: number]: { allocationLevelAndSpecialisms: string, allocationLevel: string, allocationSpecialisms: string[], forcedAllocation: boolean } } }} session
 * @returns {PageContent}
 * */
export const confirmPage = (appealDetails, appellantStatement, specialismData, session) => {
	const shortReference = appealShortReference(appealDetails.appealReference);
	const sessionData = session.acceptAppellantStatement?.[appealDetails.appealId];
							...
							items: [
								...(appellantStatement.attachments?.length > 0
									? [
											{
												text: 'Manage',
												href: `/appeals-service/appeal-details/${appealDetails.appealId}/appellant-statement/manage-documents/${folderId}?backUrl=/appellant-statement/valid/confirm`,
								...
								{
									text: 'Add',
									href: `/appeals-service/appeal-details/${appealDetails.appealId}/appellant-statement/add-document?backUrl=/appellant-statement/valid/confirm`,
									visuallyHiddenText: 'supporting documents'
								}
							]
						}
	...
	return {
		title: 'Check details and accept Appellant statement',
		backLinkUrl: `/appeals-service/appeal-details/${appealDetails.appealId}/appellant-statement/valid/allocation-specialisms`,
		preHeading: `Appeal ${shortReference}`,
		heading: 'Check details and accept Appellant statement',
		submitButtonText: 'Accept Appellant statement',
		pageComponents
	};
};
