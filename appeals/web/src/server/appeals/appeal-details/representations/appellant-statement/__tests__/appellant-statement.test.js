...
describe('appellant-statements', () => {
	...
	describe('GET /add-document', () => {
		beforeEach(() => {
			nock('http://test/').get('/appeals/2/reps').reply(200, appellantStatementAwaitingReview);
		});
	...
	describe('POST /add-document', () => {
		...
		it(`should render a 500 error page if upload-info is not present in the request body`, async () => {
			const response = await request.post(`${baseUrl}/2/appellant-statement/add-document`).send({});
		...
		it('should redirect to the add document details page if upload info is present and in the correct format', async () => {
			...
			expect(response.statusCode).toBe(302);
			expect(response.text).toBe(
				`Found. Redirecting to ${baseUrl}/2/appellant-statement/add-document/add-document-details`
			);
		});
	});
	...
	describe('GET /add-document/add-document-details', () => {
		...
		it(`should render a 500 error page if fileUploadInfo is not present in the session`, async () => {
			const response = await request.get(
				`${baseUrl}/2/appellant-statement/add-document/add-document-details`
			);
		...
		it('should render the add documents details page', async () => {
			await request.post(`${baseUrl}/2/appellant-statement/add-document`).send({
				'upload-info': fileUploadInfo
			});
	...
	describe('POST /add-document/add-document-details', () => {
		...
		it(`should re-render add documents details page if the request body is in an incorrect format`, async () => {
			const response = await request
				.post(`${baseUrl}/2/appellant-statement/add-document/add-document-details`)
				.send({});
		...
		it(`should redirect to check your answers if valid details posted`, async () => {
			...
			expect(response.text).toBe(
				`Found. Redirecting to ${baseUrl}/2/appellant-statement/add-document/check-your-answers`
			);
		});
	});
	...
	describe('GET /add-document/check-your-answers', () => {
		...
		it(`should render check your answers page with correct content`, async () => {
			const response1 = await request.post(`${baseUrl}/2/appellant-statement/add-document`).send({
				'upload-info': fileUploadInfo
			});
			...
			const response = await request.get(
				`${baseUrl}/2/appellant-statement/add-document/check-your-answers`
			);
			...
			expect(unprettifiedElement.innerHTML).toContain(
				`<a class="govuk-link" href="/appeals-service/appeal-details/2/appellant-statement/add-document">Change<span class="govuk-visually-hidden"> file test-document.txt</span></a></dd>`
			);
			...
			expect(unprettifiedElement.innerHTML).toContain(
				`<a class="govuk-link" href="/appeals-service/appeal-details/2/appellant-statement/add-document/add-document-details">Change<span class="govuk-visually-hidden"> test-document.txt date received</span></a></dd>`
			);
			expect(unprettifiedElement.innerHTML).toContain('Confirm</button>');
		});
	});
	...
	describe('POST /add-document/check-your-answers', () => {
		...
		it(`should redirect to appellant statement page`, async () => {
			await request.post(`${baseUrl}/2/appellant-statement/add-document`).send({
				'upload-info': fileUploadInfo
			});
			...
			const response = await request
				.post(`${baseUrl}/2/appellant-statement/add-document/check-your-answers`)
				.send({});

			expect(response.statusCode).toBe(302);

			expect(response.text).toBe(`Found. Redirecting to ${baseUrl}/2/appellant-statement`);
		});
	});
});
