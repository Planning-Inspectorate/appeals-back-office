...
describe('appellant statement valid', () => {
	beforeEach(() => {
		installMockApi();
	...
	describe('GET /allocation-check', () => {
		...
		it('should render a back link to the review Appellant statement page', async () => {
			nock('http://test/')
				.get(`/appeals/${appealId}?include=all`)
				.reply(200, {
					...appealDataEnforcementNotice,
			...
			expect(unprettifiedHtml).toContain(
				`href="/appeals-service/appeal-details/${appealId}/appellant-statement" class="govuk-back-link">Back</a>`
			);
		});
	});
	...
	describe('GET /allocation-level', () => {
		...
		it('should render a back link to the review Appellant statement page, if the appeal has no allocation level or allocation specialisms (i.e. review page was previous page, allocation check was skipped because user must update allocation info)', async () => {
			nock('http://test/')
				.get(`/appeals/${appealId}?include=all`)
				.reply(200, {
					...appealDataEnforcementNotice,
		...
		it('should render a back link to the allocation check page, if the appeal has both an allocation level and allocation specialisms (i.e. allocation check was previous page because user was given the option to update allocation info or not)', async () => {
			...
			expect(unprettifiedHtml).toContain(
				`href="/appeals-service/appeal-details/${appealId}/appellant-statement/valid/allocation-check" class="govuk-back-link">Back</a>`
			);
		});
	});
	...
	describe('GET /allocation-specialisms', () => {
		...
		it('should render a back link to the valid journey version of the allocation level page', async () => {
			...
			expect(innerHTML).toContain(
				`href="/appeals-service/appeal-details/${appealId}/appellant-statement/valid/allocation-level" class="govuk-back-link"`
			);
		});
	});
});
