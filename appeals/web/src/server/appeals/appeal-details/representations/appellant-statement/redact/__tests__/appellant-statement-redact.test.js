...
describe('redact', () => {
	...
	describe('GET /', () => {
		it('should render the redact appellant statement page with expected content', async () => {
			nock('http://test/')
			...
			const response = await request.get(`${baseUrl}/${appealId}/appellant-statement/redact`);
			...
			expect(innerHTML).toContain(`Redact Appellant statement</h1>`);
	...
	describe('POST /', () => {
		it(`should redirect to the redact appellant statement allocation check page if allocation details already present`, async () => {
			nock('http://test/')
				.get(`/appeals/${appealId}?include=all`)
				.reply(200, {
					...appealDataEnforcementNotice,
			...
			const response = await request
				.post(`${baseUrl}/${appealId}/appellant-statement/redact`)
				.send({
			...
			expect(response.text).toBe(
				`Found. Redirecting to /appeals-service/appeal-details/${appealId}/appellant-statement/redact/allocation-check`
			);
		});

		it(`should redirect to the redact appellant statement allocation level page if no allocation details present`, async () => {
			nock.cleanAll();
			...
			const response = await request
				.post(`${baseUrl}/${appealId}/appellant-statement/redact`)
				.send({
					redactedRepresentation: 'Test redacted appellant statement text'
				});

			expect(response.statusCode).toBe(302);
			expect(response.text).toBe(
				`Found. Redirecting to /appeals-service/appeal-details/${appealId}/appellant-statement/redact/allocation-level`
			);
		});
	});
	...
	describe('GET /allocation-check', () => {
		it('should render the allocation check page with expected content', async () => {
			...
			const response = await request.get(
				`${baseUrl}/${appealId}/appellant-statement/redact/allocation-check`
			);
			expect(response.statusCode).toBe(200);
			const { innerHTML } = parseHtml(response.text);
			expect(innerHTML).toMatchSnapshot();
		...
		it('should render a back link to the redact Appellant statement page', async () => {
			nock('http://test/')
				.get(`/appeals/${appealId}?include=all`)
				.reply(200, {
					...appealDataEnforcementNotice,
			...
			expect(unprettifiedHtml).toContain(
				`href="/appeals-service/appeal-details/${appealId}/appellant-statement/redact" class="govuk-back-link">Back</a>`
			);
		});
	});
	...
	describe('GET /allocation-level', () => {
		it('should render the allocation level page with expected content', async () => {
			...
			const response = await request.get(
				`${baseUrl}/${appealId}/appellant-statement/redact/allocation-level`
			);
			expect(response.statusCode).toBe(200);
			const { innerHTML } = parseHtml(response.text);
			expect(innerHTML).toMatchSnapshot();
		...
		it('should render a back link to the redact Appellant statement page, if the appeal has no allocation level or allocation specialisms (i.e. redact page was previous page, allocation check was skipped because user must update allocation info)', async () => {
			nock('http://test/')
				.get(`/appeals/${appealId}?include=all`)
				.reply(200, {
					...appealDataEnforcementNotice,
			...
			const reviewPageResponse = await request
				.post(`${baseUrl}/${appealId}/appellant-statement`)
				.send({
		...
		it('should render a back link to the allocation check page, if the appeal has both an allocation level and allocation specialisms (i.e. allocation check was previous page because user was given the option to update allocation info or not)', async () => {
			...
			expect(unprettifiedHtml).toContain(
				`href="/appeals-service/appeal-details/${appealId}/appellant-statement/redact/allocation-check" class="govuk-back-link">Back</a>`
			);
		});
	});
	...
	describe('GET /allocation-specialisms', () => {
		it('should render the allocation specialisms page with expected content', async () => {
			...
			const response = await request.get(
				`${baseUrl}/${appealId}/appellant-statement/redact/allocation-specialisms`
			);
			expect(response.statusCode).toBe(200);
		...
		it('should render a back link to the redact journey version of the allocation level page', async () => {
			...
			expect(innerHTML).toContain(
				`href="/appeals-service/appeal-details/${appealId}/appellant-statement/redact/allocation-level" class="govuk-back-link"`
			);
		});
	});

	describe('GET /confirm', () => {
		it('should render the CYA redaction page with expected content if Appellant statement is awaiting review', async () => {
			nock('http://test/')
				.get(`/appeals/${appealId}?include=all`)
				.reply(200, {
					...appealDataEnforcementNotice,
			...
			const response = await request.get(
				`${baseUrl}/${appealId}/appellant-statement/redact/confirm`
			);
		...
		it('should render the CYA redaction page with expected content if Appellant statement is published', async () => {
			nock.cleanAll();
