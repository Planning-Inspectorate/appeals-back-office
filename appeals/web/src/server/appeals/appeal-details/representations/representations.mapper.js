...
export function statementAndCommentsSharePage(appeal, request, backUrl) {
	...
	const nextStateDisplayText = getNextStateDisplayTextOnStatementsComplete(appeal.appealType, appeal.procedureType, hearingIsSetUp);
	...
	const warningComponent =
		appeal.procedureType?.toLowerCase() === APPEAL_CASE_PROCEDURE.INQUIRY && valueTexts.length === 0
			? {
					type: 'warning-text',
					parameters: {
						text: 'Do not progress to ' + nextStateDisplayText + ' if you are awaiting any late statements or interested party comments.'
					}
				}
			: {
					type: 'warning-text',
