...
describe('representations', () => {
	...
	describe('GET /share', () => {
		...
		describe.each([
			...
			it.each([
			...
			])('should show the correct heading and button text where procedure is %s, hearing is set up = %s and there are no statements or IP comments', async (procedureType, hearingSetUp, headingText) => {
				...
				const expectedWarningText = procedureType === APPEAL_CASE_PROCEDURE.INQUIRY ?
					`Warning</span> Do not progress to ` + headingText + ` if you are awaiting any late statements or interested party comments.</strong>` :
			...
			])('should show the correct heading and button text where procedure is %s and there are only statements', async (procedureType) => {
				const appealAtStatements = {
					...appealData,
					appealType: appealType,
					procedureType: procedureType,
			...
			it.each([
				...
				APPEAL_CASE_PROCEDURE.HEARING,
				APPEAL_CASE_PROCEDURE.INQUIRY
			])('should show the correct heading and button text where procedure is %s and there are only IP comments', async (procedureType) => {
				const appealAtStatements = {
					...appealData,
					appealType: appealType,
					procedureType: procedureType,
			...
			it.each([
				...
				APPEAL_CASE_PROCEDURE.HEARING,
				APPEAL_CASE_PROCEDURE.INQUIRY
			])('should show the correct heading and button text where procedure is %s and there are both statements and IP comments', async (procedureType) => {
				const appealAtStatements = {
					...appealData,
					appealType: appealType,
					procedureType: procedureType,
				...
				expect(textResponse.innerHTML).toContain(
					`Share comments and statements</button>`
				);
			});
		});
		...
		describe('statements status', () => {
			beforeEach(() => {
				...
				nock('http://test/')
					...
					.persist();
				nock('http://test/')
					.get('/appeals/1/reps?type=appellant_final_comment,lpa_final_comment,appellant_proofs_evidence,lpa_proofs_evidence')
			...
			it.each([
			...
			])('should show the correct banner when procedure is %s, hearing is set up = %s and there are no published representations', async (procedureType, hearingSetUp, expectedBannerText) => {
				const appealAtStatements = {
					...appealData,
					appealId,
					appealType: appealType,
					procedureType: procedureType,
					appealStatus: APPEAL_CASE_STATUS.STATEMENTS,
					hearing: hearingSetUp ? { hearingStartTime: '2026-01-01T10:00:00.000Z', address: { addressLine1: '1 Test Street', town: 'Test Town', postcode: 'AB12 3CD' } } : {},
					documentationSummary: {}
				};
				nock('http://test/')
				...
				const sharePostResponse = await request
					.post(`${baseUrl}/${appealId}/share`)
					.send({});
			...
			])('should show "Progressed to final comments" banner for all procedures where procedure is %s, hearing is set up = %s and there are no published representations', async (procedureType, hearingSetUp) => {
				const appealAtStatements = {
					...appealData,
					appealId,
					appealType: appealType,
