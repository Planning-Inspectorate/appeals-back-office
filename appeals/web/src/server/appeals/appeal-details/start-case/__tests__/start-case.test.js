...
describe('start-case', () => {
	...
	describe('GET /start-case/date', () => {
		it('"Start case" page should contain "Confirming will activate the timetable on"', async () => {
			const response = await request.get(`${baseUrl}/1/start-case/date`);
		...
		it('should redirect to the confirmation page', async () => {
			...
			const response = await request.post(`${baseUrl}/1/start-case/date`).send({});
