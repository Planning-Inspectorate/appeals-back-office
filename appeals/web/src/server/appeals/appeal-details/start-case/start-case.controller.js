import { dateToDisplayDate, dayMonthYearToApiDateString } from '#lib/dates.js';
import logger from '#lib/logger.js';
import { startCasePage, startCaseConfirmationPage } from './start-case.mapper.js';
...
export const postStartDate = async (request, response) => {
	try {
		...
		return response.redirect(`/appeals-service/appeal-details/${appealId}/start-case/confirmation`);
	} catch (error) {
		logger.error(
			error,
