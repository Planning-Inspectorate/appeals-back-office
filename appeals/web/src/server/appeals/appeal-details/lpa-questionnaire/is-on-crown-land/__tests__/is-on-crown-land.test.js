...
describe('is-on-crown-land', () => {
	(beforeEach(installMockApi), afterEach(teardown));

	describe('GET /change', () => {
		it('should render the isSiteOnCrownLand change page', async () => {
			nock('http://test/')
				.get(`/appeals/${appealId}/lpa-questionnaires/${lpaQuestionnaireId}`)
				.reply(200, lpaQuestionnaireData);
			const response = await request.get(`${lpaQuestionnaireUrl}/is-on-crown-land/change`);

			const elementInnerHtml = parseHtml(response.text).innerHTML;
			expect(response.statusCode).toEqual(200);

			expect(elementInnerHtml).toMatchSnapshot();
			expect(elementInnerHtml).toContain('Is the appeal site on Crown land?</h1>');
		});
	});
	...
	describe('POST /change', () => {
		it('should re-direct to LPA questionnaire if "yes"', async () => {
			...
			const response = await request
				.post(`${lpaQuestionnaireUrl}/is-on-crown-land/change`)
				.send(validData);
