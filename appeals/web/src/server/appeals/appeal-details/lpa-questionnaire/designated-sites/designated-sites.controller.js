 ...
 * @param {import('@pins/express/types/express.js').RenderedResponse<any, any, Number>} response
 */
export const getChangeInNearOrLikelyToAffectDesignatedSites = renderChangeInNearOrLikelyToAffectDesignatedSites;
...
export const postChangeInNearOrLikelyToAffectDesignatedSites = async (request, response) => {
	try {
		...
		session.inNearOrLikelyToAffectDesignatedSites = Array.isArray(request.body['inNearOrLikelyToAffectDesignatedSitesCheckboxes'])
			? request.body['inNearOrLikelyToAffectDesignatedSitesCheckboxes']
			: [request.body['inNearOrLikelyToAffectDesignatedSitesCheckboxes']];

		console.log('session.inNearOrLikelyToAffectDesignatedSites: ', session.inNearOrLikelyToAffectDesignatedSites);
		...
		// TODO: A2-1466: move to mapper
		/** @type {import('@pins/appeals.api/src/server/openapi-types.js').DesignatedSiteName[]} */
		const mappedInputData = session.inNearOrLikelyToAffectDesignatedSites === 'none'
			? []
			: session.inNearOrLikelyToAffectDesignatedSites.map(inputItem => {
				if (inputItem === 'other') {
					return {
						id: 0,
						key: 'custom',
				...
				const matchingDesignatedSiteName = designatedSiteNames.find(designatedSiteName => designatedSiteName.key === inputItem);

				return matchingDesignatedSiteName;
			});
