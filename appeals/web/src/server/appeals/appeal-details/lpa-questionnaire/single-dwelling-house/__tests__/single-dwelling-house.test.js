...
describe('single-dwelling-house', () => {
	(beforeEach(installMockApi), afterEach(teardown));

	describe('GET /change', () => {
		it('should render the relatesToBuildingSingleDwellingHouse change page', async () => {
			nock('http://test/')
				.get(`/appeals/${appealId}/lpa-questionnaires/${lpaQuestionnaireId}`)
				.reply(200, lpaQuestionnaireData);
			const response = await request.get(`${lpaQuestionnaireUrl}/single-dwelling-house/change`);
			...
			expect(elementInnerHtml).toContain(
				'Is the enforcement notice for a single private dwelling house?</h1>'
			);
		});
	});
	...
	describe('POST /change', () => {
		it('should re-direct to LPA questionnaire if "yes"', async () => {
			...
			const response = await request
				.post(`${lpaQuestionnaireUrl}/single-dwelling-house/change`)
				.send(validData);
