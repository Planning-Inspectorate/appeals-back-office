...
const renderAddAffectedListedBuilding = async (request, response) => {
	...
	const mappedPageContents = addAffectedListedBuildingPage(currentAppeal, request.session.affectedListedBuilding);
...
const renderAddAffectedListedBuildingCheckAndConfirm = async (request, response) => {
	...
	const mappedPageContent = addAffectedListedBuildingCheckAndConfirmPage(currentAppeal, session.affectedListedBuilding);
...
export const postAddAffectedListedBuildingCheckAndConfirm = async (request, response) => {
	...
	try {
		...
		return response.redirect(`/appeals-service/appeal-details/${appealId}/lpa-questionnaire/${lpaQuestionnaireId}`);
	} catch (error) {
		logger.error(error);
	}
...
const renderManageAffectedListedBuildings = async (request, response) => {
	...
	const mappedPageContents = manageAffectedListedBuildingPage(lpaQuestionnaire);

	return response.status(200).render('pattern/display-page.pattern.njk', {
...
const renderRemoveAffectedListedBuilding = async (request, response) => {
	...
	const lpaQuestionnaire = await getLpaQuestionnaireFromId(apiClient, appealId, lpaQuestionnaireId);
	const mappedPageContents = removeAffectedListedBuildingPage(lpaQuestionnaire, listedBuildingId);
...
export const postRemoveAffectedListedBuilding = async (request, response) => {
	...
	if (!body['removeAffectedListedBuilding'] || !appealId || !listedBuildingId || !lpaQuestionnaireId ) {
		return response.status(500).render('app/500.njk');
	}

	if(body['removeAffectedListedBuilding'] === 'no') {
		return response.redirect(`/appeals-service/appeal-details/${appealId}/lpa-questionnaire/${lpaQuestionnaireId}/affected-listed-buildings/manage`);
	} else if(body['removeAffectedListedBuilding'] === 'yes') {
		await removeAffectedListedBuilding(apiClient, appealId, lpaQuestionnaireId, listedBuildingId);
		addNotificationBannerToSession(
			request.session,
			'affectedListedBuilding',
			appealId,
		...
		if(lpaQuestionnaire.listedBuildingDetails && lpaQuestionnaire.listedBuildingDetails.length > 0){
			return response.redirect(`/appeals-service/appeal-details/${appealId}/lpa-questionnaire/${lpaQuestionnaireId}/affected-listed-buildings/manage`);
		} else {
			return response.redirect(`/appeals-service/appeal-details/${appealId}/lpa-questionnaire/${lpaQuestionnaireId}`);
		}
...
const renderChangeAffectedListedBuilding = async (request, response) => {
	...
	const mappedPageContents = changeAffectedListedBuildingPage(lpaQuestionnaire, listedBuildingId);
...
export const postChangeAffectedListedBuilding = async (request, response) => {
	...
	return response.redirect(`/appeals-service/appeal-details/${appealId}/lpa-questionnaire/${lpaQuestionnaireId}/affected-listed-buildings/change/${listedBuildingId}/check-and-confirm`)
}
