...
describe('change-of-use-mineral-storage', () => {
	(beforeEach(installMockApi), afterEach(teardown));

	describe('GET /change', () => {
		it('should render the changeOfUseMineralStorageRouter change page', async () => {
			nock('http://test/')
				.get(`/appeals/${appealId}/lpa-questionnaires/${lpaQuestionnaireId}`)
				.reply(200, lpaQuestionnaireData);
			const response = await request.get(
				`${lpaQuestionnaireUrl}/change-of-use-mineral-storage/change`
			);
			...
			expect(elementInnerHtml).toContain(
				'Does the enforcement notice include a change of use of land to store minerals in the open?</h1>'
			);
		});
	});
	...
	describe('POST /change', () => {
		it('should re-direct to LPA questionnaire if "yes"', async () => {
			const validData = {
				changeOfUseMineralStorageRadio: 'yes'
			};
			...
			const response = await request
				.post(`${lpaQuestionnaireUrl}/change-of-use-mineral-storage/change`)
				.send(validData);
		...
		it('should re-direct to LPA questionnaire if "no"', async () => {
			const validData = {
				changeOfUseMineralStorageRadio: 'no'
			};
