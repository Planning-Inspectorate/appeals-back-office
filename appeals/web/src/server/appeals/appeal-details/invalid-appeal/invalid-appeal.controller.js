...
export const postEnforcementNoticeInvalid = async (request, response) => {
	...
	try {
		...
		if (session.enforcementDecision.enforcementNoticeInvalid === 'no') {
			delete session.enforcementDecision.enforcementNoticeReason;
			return response.redirect(
...
export const postEnforcementNoticeReason = async (request, response) => {
	...
	try {
		...
		return response.redirect(
			`/appeals-service/appeal-details/${currentAppeal.appealId}/appellant-case/invalid/other-information`
		);
	} catch (error) {
		logger.error(
			error,
	...
	if (invalidReasonOptions) {
		const pageContent = enforcementNoticeReasonPage(
			...
			invalidReasonOptions.map((option) => {
				...
				const reason = session?.enforcementDecision?.enforcementNoticeReason?.find(
