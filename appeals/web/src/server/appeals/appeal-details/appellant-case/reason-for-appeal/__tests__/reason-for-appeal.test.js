...
describe('development-description', () => {
	...
	describe('GET /change', () => {
		it('should render the reason for appeal change page when accessed from the appellant case page', async () => {
			nock('http://test/')
				.get(`/appeals/${appealData.appealId}/appellant-cases/${appealData.appellantCaseId}`)
				.reply(200, { appellantCaseDataIncompleteOutcome });
			const response = await request.get(`${baseUrl}/reason-for-appeal/change`);
	...
	describe('POST /change', () => {
		it('should re-direct to appellant case when the data is valid', async () => {
			const apiCall = nock('http://test/')
				...
				.reply(200, {});
			const validData = {
				reasonForAppealAppellant: 'Some valid text'
			};

			const response = await request.post(`${baseUrl}/reason-for-appeal/change`).send(validData);
		...
		it('should re-render the reason for appeal change page with an error if the field is empty', async () => {
			const apiCall = nock('http://test/')
				.patch(`/appeals/${appealData.appealId}/appellant-cases/${appealData.appellantCaseId}`)
				.reply(200, {});
