...
describe('application-submission-date', () => {
	...
	describe('GET /change', () => {
		it('should render the applicationSubmissionDate change page', async () => {
			...
			expect(elementInnerHtml).toContain('What date did you submit your application?</h1>');
		});
	});
		...
		it('should re-render the Date page with an error message if the provided date month is invalid', async () => {
			const testCases = [
				{ value: '', expectedError: 'Date must include a month' },
				{
					value: 'a',
					expectedError:
						'Date month must be a number, a full month name or a three letter abbreviation for a month'
				},
