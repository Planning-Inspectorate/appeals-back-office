...
const hearingDocsFolderId = hearingDocumentsFolderInfo.folderId;
...
describe('hearing documents', () => {
	...
	beforeEach(() => {
		...
		nock('http://test/')
			...
			.persist();
		nock('http://test/').get(getFolderApiUrl(1)).reply(200, hearingDocumentsFolderInfo).persist();
	...
	describe('hearing documents', () => {
		describe('GET /hearing-documents/upload-documents/:folderId', () => {
			it(`should render the upload documents page`, async () => {
				const response = await request.get(
					`${baseUrl}/1/hearing-documents/upload-documents/${hearingDocsFolderId}`
				);
				...
				expect(unprettifiedElement.innerHTML).toContain(`Upload hearing document</h1>`);
		...
		describe('POST /hearing-documents/upload-documents/:folderId', () => {
			it(`should render a 500 error page if upload-info is not present in the request body`, async () => {
				const response = await request
					.post(`${baseUrl}/1/hearing-documents/upload-documents/${hearingDocsFolderId}`)
					.send({});
			...
			it(`should redirect to the add document details page if upload-info is present in the request body and in the correct format`, async () => {
				...
				expect(response.text).toBe(
					`Found. Redirecting to /appeals-service/appeal-details/1/hearing-documents/add-document-details/${hearingDocsFolderId}`
				);
			});
		});

		describe('GET /hearing-documents/upload-documents/:folderId/:documentId', () => {
			beforeEach(() => {
				nock('http://test/')
			...
			it(`should render the upload document version page for document`, async () => {
				const response = await request.get(
					`${baseUrl}/1/hearing-documents/upload-documents/${hearingDocsFolderId}/1`
				);
				const element = parseHtml(response.text);

				expect(element.innerHTML).toMatchSnapshot();
				expect(element.innerHTML).toContain(`Hearing documents</h1>`);
		...
		describe('POST /hearing-documents/upload-documents/:folderId/:documentId', () => {
			it(`should render a 500 error page if upload-info is not present in the request body`, async () => {
				const response = await request
					.post(`${baseUrl}/1/hearing-documents/upload-documents/${hearingDocsFolderId}/1`)
					.send({});
			...
			it(`should redirect to the add document details page if upload-info is present in the request body and in the correct format`, async () => {
				...
				expect(response.statusCode).toBe(302);
				expect(response.text).toBe(
					`Found. Redirecting to /appeals-service/appeal-details/1/hearing-documents/add-document-details/${hearingDocsFolderId}/1`
				);
			});
		});

		describe('GET /hearing-documents/add-document-details/:folderId', () => {
			it(`should render a 500 error page if fileUploadInfo is not present in the session`, async () => {
				const response = await request.get(
					`${baseUrl}/1/hearing-documents/add-document-details/${hearingDocsFolderId}`
				);
			...
			it(`should render the document details page with one item per uploaded document`, async () => {
				...
				expect(unprettifiedElement.innerHTML).toContain(`Hearing document</h1>`);
			...
			it(`should render a back link to the upload document page`, async () => {
				...
				expect(element.innerHTML).toContain(
					`href="/appeals-service/appeal-details/1/hearing-documents/upload-documents/${hearingDocsFolderId}"`
				);
			});
		});

		describe('POST /hearing-documents/add-document-details/:folderId', () => {
			/**
			...
			let expectedH1Text = `Hearing document`;
			...
			it(`should re-render the document details page with the expected error message if the request body is in an incorrect format`, async () => {
				...
				const response = await request
					.post(`${baseUrl}/1/hearing-documents/add-document-details/${hearingDocsFolderId}`)
					.send({});
			...
			it(`should re-render the document details page with the expected error message if receivedDate day is an invalid value`, async () => {
				...
				const testCases = [
					{
						value: '',
						expectedError: `Hearing document date must include a day`
					},
					{
						value: 'a',
						expectedError: `Hearing document date day must be a number`
					},
					{
						value: '0',
						expectedError: `Hearing document date day must be between 1 and 31`
					},
					{
						value: '32',
						expectedError: `Hearing document date day must be between 1 and 31`
			...
			it(`should re-render the document details page with the expected error message if receivedDate month is an invalid value`, async () => {
				...
				const testCases = [
					{
						value: '',
						expectedError: `Hearing document date must include a month`
					},
					{
						value: 'a',
						expectedError: `Hearing document date must be a real date`
					},
					{
						value: '0',
						expectedError: `Hearing document date month must be between 1 and 12`
					},
					{
						value: '13',
						expectedError: `Hearing document date month must be between 1 and 12`
			...
			it(`should re-render the document details page with the expected error message if receivedDate year is an invalid value`, async () => {
				...
				const testCases = [
					{
						value: '',
						expectedError: `Hearing document date must include a year`
					},
					{
						value: 'a',
						expectedError: `Hearing document date year must be a number`
					},
					{
						value: '202',
						expectedError: `Hearing document date year must be 4 digits`
					}
				];
			...
			it(`should re-render the document details page with the expected error message if receivedDate is not a valid date`, async () => {
				...
				expect(errorSummaryElement.innerHTML).toContain(
					`Hearing document date must be a real date`
				);
			});
			...
			it(`should send a patch request to the appeal documents endpoint and redirect to the check and confirm page, if complete and valid document details were provided`, async () => {
				...
				expect(response.text).toEqual(
					`Found. Redirecting to /appeals-service/appeal-details/1/hearing-documents/check-your-answers/${hearingDocsFolderId}`
				);
			});
		});

		describe('GET /hearing-documents/add-document-details/:folderId/:documentId', () => {
			it(`should render a 500 error page if fileUploadInfo is not present in the session`, async () => {
				const response = await request.get(
					`${baseUrl}/1/hearing-documents/add-document-details/${hearingDocsFolderId}/1`
				);
			...
			it(`should render a back link to the upload document version page`, async () => {
				...
				expect(element.innerHTML).toContain(
					`href="/appeals-service/appeal-details/1/hearing-documents/upload-documents/${hearingDocsFolderId}/1"`
				);
			});
		});

		describe('POST /hearing-documents/add-document-details/:folderId/:documentId', () => {
			/**
			 * @type {import("superagent").Response}
			 */
			let addDocumentsResponse;
			...
			it(`should re-render the document details page with the expected error message if the request body is in an incorrect format`, async () => {
				...
				const response = await request
					.post(`${baseUrl}/1/hearing-documents/add-document-details/${hearingDocsFolderId}/1`)
					.send({});
			...
			it(`should send a patch request to the appeal documents endpoint and redirect to the check and confirm page, if complete and valid document details were provided`, async () => {
				...
				expect(response.statusCode).toBe(302);
				expect(response.text).toEqual(
					`Found. Redirecting to /appeals-service/appeal-details/1/hearing-documents/check-your-answers/${hearingDocsFolderId}/1`
				);
			});
		});

		describe('GET /hearing-documents/check-your-answers/:folderId', () => {
			it(`should render a 500 error page if fileUploadInfo is not present in the session`, async () => {
				const response = await request.get(
					`${baseUrl}/1/hearing-documents/check-your-answers/${hearingDocsFolderId}`
				);
			...
			it(`should render the add documents check and confirm page with summary list displaying info on the uploaded document and relevant change links`, async () => {
				...
				expect(unprettifiedElement.innerHTML).toContain(
					`<a class="govuk-link" href="/appeals-service/appeal-details/1/hearing-documents/upload-documents/${hearingDocsFolderId}">Change<span class="govuk-visually-hidden"> file test-document.txt</span></a></dd>`
				);
				...
				expect(unprettifiedElement.innerHTML).toContain(
					`<a class="govuk-link" href="/appeals-service/appeal-details/1/hearing-documents/add-document-details/${hearingDocsFolderId}">Change<span class="govuk-visually-hidden"> test-document.txt date received</span></a></dd>`
				);
				expect(unprettifiedElement.innerHTML).toContain(
					`<a class="govuk-link" href="/appeals-service/appeal-details/1/hearing-documents/add-document-details/${hearingDocsFolderId}">Change<span class="govuk-visually-hidden"> test-document.txt redaction status</span></a></dd>`
				);
				expect(unprettifiedElement.innerHTML).toContain('Confirm</button>');
			});
		});

		describe('POST /hearing-documents/check-your-answers/:folderId', () => {
			it(`should render a 500 error page if fileUploadInfo is not present in the session`, async () => {
				const response = await request
					.post(`${baseUrl}/1/hearing-documents/check-your-answers/${hearingDocsFolderId}`)
					.send({});
		...
		describe('GET /hearing-documents/check-your-answers/:folderId/:documentId', () => {
			it(`should render a 500 error page if fileUploadInfo is not present in the session`, async () => {
				const response = await request.get(
					`${baseUrl}/1/hearing-documents/check-your-answers/${hearingDocsFolderId}/1`
				);
			...
			it(`should render the add documents check and confirm page with summary list row displaying info on the uploaded document`, async () => {
				...
				expect(unprettifiedElement.innerHTML).toContain(
					`<a class="govuk-link" href="/appeals-service/appeal-details/1/hearing-documents/upload-documents/${hearingDocsFolderId}/1">Change<span class="govuk-visually-hidden"> file test-document.txt</span></a></dd>`
				);
				expect(unprettifiedElement.innerHTML).toContain('Date received</dt>');
				expect(unprettifiedElement.innerHTML).toContain(
					`${dateISOStringToDisplayDate(new Date().toISOString())}</dd>`
				...
				expect(unprettifiedElement.innerHTML).toContain('No redaction required</dd>');
				expect(unprettifiedElement.innerHTML).toContain(
					`<a class="govuk-link" href="/appeals-service/appeal-details/1/hearing-documents/add-document-details/${hearingDocsFolderId}/1">Change<span class="govuk-visually-hidden"> test-document.txt date received</span></a></dd>`
				);
				expect(unprettifiedElement.innerHTML).toContain(
					`<a class="govuk-link" href="/appeals-service/appeal-details/1/hearing-documents/add-document-details/${hearingDocsFolderId}/1">Change<span class="govuk-visually-hidden"> test-document.txt redaction status</span></a></dd>`
				);
				expect(unprettifiedElement.innerHTML).toContain('Confirm</button>');
			});
		});

		describe('POST /hearing-documents/check-your-answers/:folderId/:documentId', () => {
			it(`should render a 500 error page if fileUploadInfo is not present in the session`, async () => {
				const response = await request
					.post(`${baseUrl}/1/hearing-documents/check-your-answers/${hearingDocsFolderId}/1`)
					.send({});
		...
		describe('GET /hearing-documents/manage-documents/:folderId', () => {
			beforeEach(() => {
				usersService.getUserByRoleAndId = jest.fn().mockResolvedValue(activeDirectoryUsersData[0]);
			});

			it(`should render a 404 error page if the folderId is not valid`, async () => {
				const response = await request.get(`${baseUrl}/1/hearing-documents/manage-documents/99`);
				const element = parseHtml(response.text);
			...
			it(`should render the manage folder page with one document item for each document present in the folder if the folderId is valid`, async () => {
				const response = await request.get(
					`${baseUrl}/1/hearing-documents/manage-documents/${hearingDocsFolderId}`
				);
				const element = parseHtml(response.text);
				...
				expect(unprettifiedElement.innerHTML).toContain(`Hearing documents</h1>`);
				expect(unprettifiedElement.innerHTML).toContain(
					`<a href="/appeals-service/appeal-details/1/hearing-documents/upload-documents/${hearingDocsFolderId}" role="button" draggable="false" class="govuk-button govuk-button--secondary" data-module="govuk-button"> Add document</a>`
				);
			});
		});
		describe('GET /hearing-documents/manage-documents/:folderId/:documentId', () => {
			beforeEach(() => {
			...
			it(`should render a 404 error page if the folderId is not valid`, async () => {
				...
				const response = await request.get(`${baseUrl}/1/hearing-documents/manage-documents/99/1`);
				const element = parseHtml(response.text);
			...
			it(`should render a 404 error page if the documentId is not valid`, async () => {
				...
				const response = await request.get(
					`${baseUrl}/1/hearing-documents/manage-documents/${hearingDocsFolderId}/99`
				);
				const element = parseHtml(response.text);
			...
			it(`should render 'Shared' tags under the Version Summary and in Version History if document IS shared`, async () => {
				...
				nock('http://test/')
					...
					.reply(200, hearingDocumentsFolderInfo);
				nock('http://test/').get('/appeals/documents/1').reply(200, documentFileInfo);
		...
		describe('GET /hearing-documents/manage-documents/:folderId/:documentId/:versionId/delete', () => {
			it(`should render the delete document page with the expected content when there is a single document version`, async () => {
				...
				const response = await request.get(
					`${baseUrl}/1/hearing-documents/manage-documents/${hearingDocsFolderId}/1/1/delete`
				);
		...
		describe('POST /hearing-documents/manage-documents/:folderId/:documentId/:versionId/delete', () => {
			beforeEach(() => {
			...
			it(`should re-render the delete document page with the expected error message if answer was not provided`, async () => {
				...
				const response = await request
					.post(`${baseUrl}/1/hearing-documents/manage-documents/${hearingDocsFolderId}/1/1/delete`)
					.send({});
				const element = parseHtml(response.text);
			...
			it(`should not send an API request to delete the document, and should redirect to the manage document page, if answer "no" was provided`, async () => {
				...
				expect(response.text).toContain(
					`/appeals-service/appeal-details/1/hearing-documents/manage-documents/${hearingDocsFolderId}/1`
				);
			});
		...
		describe('GET /hearing-documents/change-document-name/:folderId/:documentId', () => {
			beforeEach(() => {
				nock('http://test/')
					.get('/appeals/documents/1/versions')
					.reply(200, documentFileVersionsInfoChecked);
			...
			it('should render the change document name page with the expected content', async () => {
				const response = await request.get(
					`${baseUrl}/1/hearing-documents/change-document-name/1/1`
				);
				const element = parseHtml(response.text);
		...
		describe('POST /hearing-documents/change-document-name/:folderId/:documentId', () => {
			beforeEach(() => {
			...
			it('should redirect to manage documents page after change document name success', async () => {
				const fullUrl = `/appeals-service/appeal-details/1/hearing-documents/change-document-name/1/1`;
				const response = await request
					.post(`${baseUrl}/1/hearing-documents/change-document-name/1/1`)
	...
	describe('GET and POST /hearing-documents/manage-documents/:folderId/:documentId/invite-main-party-comments', () => {
		...
		it(`should render the invite main party comments page`, async () => {
			const response = await request.get(
				`${baseUrl}/1/hearing-documents/manage-documents/${hearingDocsFolderId}/1/invite-main-party-comments`
			);
		...
		it(`should redirect to check-your-answers if an option is selected on POST`, async () => {
			...
			expect(response.statusCode).toBe(302);
			expect(response.text).toContain(
				`Found. Redirecting to /appeals-service/appeal-details/1/hearing-documents/manage-documents/${hearingDocsFolderId}/1/check-your-answers`
			);
		});
	});
