...
describe('change-appeal-type', () => {
	...
	it('should re-render the final date page with an error message if required field is missing', async () => {
		...
		expect(unprettifiedErrorSummaryHTML).toContain('There is a problem</h2>');
		expect(unprettifiedErrorSummaryHTML).toContain(
			'Deadline to resubmit the appeal must include a day'
		);
		expect(unprettifiedErrorSummaryHTML).toContain(
			'Deadline to resubmit the appeal must include a month'
		);
		expect(unprettifiedErrorSummaryHTML).toContain(
			'Deadline to resubmit the appeal must include a year'
		);
	});
