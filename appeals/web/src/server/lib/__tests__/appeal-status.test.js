...
describe('getNextStateDisplayTextOnStatementsComplete', () => {
	...
	describe.each([
		...
		])('returns %s when procedure is %s and hearing is set up = %s', async (expectedText, procedureType, hearingSetUp) => {
			expect(getNextStateDisplayTextOnStatementsComplete(appealType, procedureType, hearingSetUp)).toBe(
				expectedText
			);
		});
	});
