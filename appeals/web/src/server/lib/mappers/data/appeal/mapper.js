 ...
 * @property {WebAppeal|LPAQuestionnaire} appealDetails
...
const submaps = {
	...
	[APPEAL_TYPE.ENFORCEMENT_LISTED_BUILDING]: s78Submaps
};

const caseSummarySubmappers = {
...
/**
 * @param {WebAppeal|LPAQuestionnaire} appealDetails
 ...
 * @returns {Promise<{appeal: MappedInstructions}>}
 */
export async function initialiseAndMapAppealDataForCaseSummary(
	appealDetails,
	currentRoute,
	session,
	request
	...
	/** @type {{appeal: MappedInstructions}} */
	const mappedData = { appeal: {} };
	Object.entries(caseSummarySubmappers).forEach(([key, submapper]) => {
		mappedData.appeal[key] = submapper({
			appealDetails,
			currentRoute: stripQueryString(currentRoute),
			session,
