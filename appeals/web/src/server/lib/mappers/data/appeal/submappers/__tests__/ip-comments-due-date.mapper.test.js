...
describe.each([
...
])('ip-comments-due-date.mapper - %s', (_, appealType) => {
	let data;
	beforeEach(() => {
		data = {
			currentRoute: '/test',
	...
	it(`should display IP Comments due date with Change action link when appeal has started and undefined published IP comments in documentation summary- ${appealType}`, () => {
		data.appealDetails.startedAt = '2025-01-01';
		data.appealDetails.procedureType = 'written';
		data.appealDetails.appealType = appealType;
	...
	it(`should display IP Comments due date with no Change action link when appeal has started and published IP comments (we will have completed the statements status)- ${appealType}`, () => {
		data.appealDetails.startedAt = '2025-01-01';
		data.appealDetails.procedureType = 'written';
		data.appealDetails.appealType = appealType;
		data.appealDetails.completedStateList = ['statements'];
		const mappedData = mapIpCommentsDueDate(data);
		expect(mappedData).toEqual({
			display: {
				summaryListItem: {
