...
describe('mapStatusDependentNotifications', () => {
	...
	for (const testCase of testCases) {
		it(`should return "${testCase.bannerKey}" banner if getRequiredActionsForAppeal returns "${testCase.requiredAction}"`, async () => {
			...
			const { mapStatusDependentNotifications } = await import(
				'#lib/mappers/utils/map-status-dependent-notifications.js'
			);

			const result = mapStatusDependentNotifications(mockAppealData);
