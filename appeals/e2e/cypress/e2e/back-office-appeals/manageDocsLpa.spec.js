...
import { isLocal } from '../../page_objects/shared.js';
	...
	it('can mark lpa proof of evidence as incomplete and select reason', { tags: tag.smoke }, () => {
		cy.createCase({ caseType: 'W' }).then((caseRef) => {
			cy.getBusinessActualDate(new Date(), 28).then((inquiryDate) => {
				...
				const reason = isLocal() ? 'Not complete' : 'Contains links to web pages';
				evidenceReasonsSection.selectEvidenceReasonOption(reason);
