...
export class CaseDetailsPage extends Page {
	...
	checkAnswerRequestDate() {
		//creates an instance of the current date and stores the day, month and year in seperate variables
		const months = [
			'January',
			'February',
			'March',
		...
		let answerDay = todaysDate.getDate();
		let answerMonth = months[todaysDate.getMonth()];
		let answerYear = todaysDate.getFullYear();

		// Captures the element that displays the Request date on the check your answer page
		let answer = cy
			.get('.govuk-summary-list__key')
		...
		//confirms the date displayed contains the day, month and year generated by the test
		answer.should('contain', answerDay);
		answer.should('contain', answerMonth);
		answer.should('contain', answerYear);
	}
