...
export const mapCase = ({ appeal, appealTypes = [], context = contextEnum.appealDetails }) => {
	if (context && appeal?.id && appeal?.caseCreatedDate) {
		const caseMap = createCaseMap({ appeal, appealTypes, context });
		return createCaseLayout(caseMap, { appeal, appealTypes, context });
	}
};
 ...
 * @returns {Map<string, *>}
 */
function createCaseMap(mappingRequest) {
 ...
 */
function updateMap(existingMap, newMap) {
 ...
 * @param {MappingRequest} mappingRequest
 */
function createCaseLayout(caseMap, mappingRequest) {
	const {
		appealSummary,
		team,
		appealRelationships,
